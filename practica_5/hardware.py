...
DIAGRAMA_INTERRUPTION_TYPE = "#DIAGRAMA"
...
class Cpu():
    ...
    def tick(self, tickNbr):
        if (self.isBusy()):
            self._fetch()
            self._decode()
            self._execute(tickNbr)
        else:
            #No esta funcionando
            ##self._interruptVector.handle(IRQ(DIAGRAMA_INTERRUPTION_TYPE, {"PC": self.pc - 1, "TICK": tickNbr}))
...
class Timer():
    def __init__(self,cpu,interruptVector):
            self._cpu = cpu
            self._interruptVector = interruptVector
            self._tickCount = 0 #los ciclos ejecutados
    ...
    def tick(self):
        ...
        if self._active and (self._tickCount > self._quantum) and self._cpu.isBusy():
            ...
            self._cpu.tick()
    # context switch
    # para la ejecucion de un proceso para dar paso a la ejecucion de otro distinto.
    def reset(self):
        self._tickCount = 0
class Hardware():
